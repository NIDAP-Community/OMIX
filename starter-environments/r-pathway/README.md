...
dedicated runtime library from source archives, in dependency order. This
avoids binary restore-order differences across package-manager endpoints.
The restore uses the CRAN source archive and the matching Bioconductor 3.20
repositories explicitly; it does not depend on a mutable package-manager
endpoint.
